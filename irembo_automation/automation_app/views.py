...
def run_automation_worker(application_id):
    ...
    try:
        with sync_playwright() as p:
            ...
            # Keep headless=False on your Ubuntu ThinkPad for testing visibility.
            # This can be set to True on the client's production Windows 11 machine.
            engine.initialize_stealth_browser(p, headless=False)
            
            # Map identity fields directly from your production model schema
            # Using first_name as the validation fallback target
            engine.navigate_to_booking_form(
                national_id=application.national_id,
                verification_data=application.first_name,
                service_type="BURANDU" # Hardcoded or matched via application fields
            )
            
            # Execute background loop. Toggles into Cooperative Interrupt automatically if slot matches
        ...
        # Safeguard fallback: Don't overwrite state if it already managed to finish successfully
